...
class UserProfile extends React.Component {
    constructor(props) {
        ...
        this.state = {
            tableHeader: ["ID", "Nombre", "Razón Social", "Descripción", "Dirección"],
            endpointUrl: `http://${REACT_APP_SERVER_URL}/providers`,
            tableData: [],
            reportData: [],
        };
    ...
    fillTable(e) {
        axios.get(this.state.endpointUrl)
            .then(res => {
                this.setState({reportData: res.data.providers});
                this.setState({tableData: res.data.providers.map(c => Object.values(c))});
            })
    }
                    ...
                    <GridItem xs={12} sm={12} md={12}>
                            <Card>
                                <CardHeader color="info">
                                    <h4 className={classes.cardTitleWhite}>Listado de Proveedores</h4>
                                </CardHeader>
                                <CardBody>
                                    ...
                                    <GridContainer>
                                        <GridItem xs={3} sm={3} md={3}>
                                            <Button type="button" color="info" size="xs"
                                                    onClick={() => generatePDFProviders(this.state.reportData)}>
                                                Generar PDF
                                            </Button>
                                        </GridItem>
                                    </GridContainer>
