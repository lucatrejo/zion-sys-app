...
class UserProfile extends React.Component {
    constructor(props) {
        ...
        this.state = {
            tableHeader: ["ID", "Nombre", "Cantidad", "Stock"],
            endpointUrl: `http://${REACT_APP_SERVER_URL}/purchases/top_items`,
            tableData: [],
            reportData: [],
        };
    ...
    fillTable(e) {
        axios.get(this.state.endpointUrl)
            .then(res => {
                this.setState({reportData: res.data.top_items});
                this.setState({tableData: res.data.top_items.map(c => Object.values(c))});
            })
    }
                        ...
                        <form onSubmit={this.insertObject}>
                            <Card>
                                <CardHeader color="info">
                                    <h4 className={classes.cardTitleWhite}>Top de Artículos mas comprados</h4>
                                </CardHeader>
                                <CardBody>
